...
class EVAObjectModelUtilsTest {
     ...
     */
    private static SubmittedVariant realWorldSV = new SubmittedVariant("GCA_000002285.2", 9615,
            "BROAD_DBSNP.2005.2.4.16.57", "CM000028.3", 41173146, "C", "G", 853087266L, true, true, true, true, null)
    private static SubmittedVariantEntity realWorldSVE =
    ...
    private static ClusteredVariant realWorldCV = new ClusteredVariant("GCA_000002285.2", 9615,
            "CM000028.3", 41173146, VariantType.SNV, true, null)
    private static ClusteredVariantEntity realWorldCVE =
            new ClusteredVariantEntity(853087266L, "8A2D7C34562DAC4B6FB824E2CD28D98D0FDAECB2", realWorldCV, 1)

    @Test
    void testGetClusteredVariantHash() {
        assertEquals(realWorldCVE.hashedMessage, getClusteredVariantHash(realWorldSV))
    }
    ...
    @Test
    void testSubmittedVariantEntityToClusteredVariantEntity() {
        assertEquals(realWorldCVE, toClusteredVariantEntity(realWorldSVE))
    }

    @Test
    void testClusteredVariantToClusteredVariantEntity() {
        assertEquals(realWorldCVE, toClusteredVariantEntity(realWorldCVE.accession, realWorldCV))
    }

    @Test
    void testSubmittedVariantToSubmittedVariantEntity() {
        assertEquals(realWorldSVE, toSubmittedVariantEntity(realWorldSVE.accession, realWorldSV))
    }
}
